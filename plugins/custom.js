window.onload = function () {
  const offset = document.querySelector('#chimenu').offsetHeight;
  let sections = {};
  ...
  let scrollSpy = () => {
    ...
    for (const key in sections) {
      if (sections[key].top <= viewTop && viewTop <= sections[key].bottom) {
        let turnOff = document.querySelector(`#chimenu a.active:not([href="#${key}"])`);
        if (turnOff) turnOff.classList.remove('active'); //如果存在持有.active但不是持有href=key的人，取消他的active
        let turnOn = document.querySelector(`#chimenu a[href="#${key}"]:not(.active)`);
...
document.querySelectorAll("#chimenu a,footer a").forEach(e => {
...
let indexShown = () => {
  const
    ...
    indexBottom = document.querySelector('#chislider').offsetHeight,
    targetMenu = document.querySelector('#chimenu'),
    targetArrow = document.querySelector('#chiarrow'),
    startY = document.scrollingElement.scrollTop; // 新語法可跨平台
